...
| # | Item | Priority |
|---|---|---|
| DE-0 | `OI_ApexClassScanner` REFERENCES-edge extraction (regex/tokenizing heuristic against `ApexClass.Body`, graceful degradation without Author Apex access) — the minimum-viable data source DE-1+ needs to have anything real to traverse | P0 — done |
| DE-0a | **Accuracy fix (2026-08-19):** strip comments and string literals before identifier tokenizing (`OI_ApexClassScanner.stripNonCodeRegions`). Previously a class named only in a doc comment or an error-message string produced a real-looking dependency edge — and because this codebase's own convention is doc headers that list collaborating classes by name, *the better-documented a class was, the more phantom dependencies it appeared to have*. Uses one alternation pass so `//` inside a string doesn't start a comment and an apostrophe inside a comment doesn't open a string; regions collapse to a space so adjacent identifiers can't fuse. The other heuristic limits (dynamic Apex, interface indirection, namespaced-name collisions) are unchanged and still need DE-15 | P0 — done |
...
| DE-7 | Impact-cache invalidation tied to scan deltas — **wired into the live scan pipeline 2026-08-19**. Previously the mechanism existed but had exactly one caller: its own unit test, so Impact Analysis served cached results indefinitely after a rescan (a correctness defect, not a missing feature — users got confidently wrong answers). Now `OI_GraphBuilder.ingest` returns an `OI_IngestDelta` reporting what genuinely changed (never liveness touches), accumulated by `OI_MutationGenerator` and consumed at all four ingest sites: the inline orchestrator path plus the component/relationship/retire Batchables. Two defects were found and fixed by the act of wiring it: (1) a changed **edge** must invalidate **both endpoint nodes**, since an edge change alters impact answers for a node whose own content never changed — invalidating only changed nodes left the target serving stale results; (2) `invalidateForScanDelta` did 2 SOQL per changed key, which would have exceeded the 100-query limit on any real scan chunk (60 keys → 120 queries) — replaced with a single bulk 1-hop expansion (`OI_GraphEngine.getOneHopNeighborKeys`), 2 queries regardless of set size. Invalidation is best-effort and logged: a cache-eviction failure must never fail the scan that produced the data | P1 — done |
