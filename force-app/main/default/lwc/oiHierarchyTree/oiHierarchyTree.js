...
export default class OiHierarchyTree extends LightningElement {
  ...
  async maybeLoadDescendants() {
    ...
    try {
      const result = await getDescendants({
        definitionId: this._definitionId,
        parentObjectApiName: this._rootObjectApiName,
        parentRecordId: this._rootRecordId
      });
      ...
      this.errorMessage =
        ...
        "Something went wrong loading the hierarchy tree. Please try again.";
    } finally {
  ...
  buildChildrenMap(descendants) {
    ...
    for (const relationship of descendants || []) {
      ...
      siblings.push({
        recordId: relationship.childRecordId,
        objectApiName: relationship.childObjectApiName,
        label: relationship.childLabel || "(not visible)"
      });
  ...
  /** Retry is meaningful here: the three inputs to the fetch are already held in state, so re-running it is the whole recovery. */
  handleRetry() {
    this.maybeLoadDescendants();
  }
  ...
  handleRootSelect(event) {
    this.dispatchEvent(
      new CustomEvent("recordselect", { detail: event.detail })
    );
  }
}
