...
jest.mock(
  "@salesforce/apex/OI_HierarchyQueryController.getDescendants",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
...
describe("c-oi-hierarchy-tree", () => {
  ...
  it("builds a parent-to-children adjacency map from the flat relationship list and renders the root node", async () => {
    ...
    const element = createElement("c-oi-hierarchy-tree", {
      is: OiHierarchyTree
    });
  ...
  it("shows the hasMore note honestly instead of silently truncating", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="tree-has-more-note"]')
    ).not.toBeNull();
  });
  ...
  it("re-emits recordselect bubbled up from the tree node as its own recordselect", async () => {
    ...
    root.shadowRoot
      .querySelector("c-oi-hierarchy-tree-node")
      .shadowRoot.querySelector('[data-id="tree-node-label"]')
      .click();

    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({
        detail: { objectApiName: "Account", recordId: "001x2" }
      })
    );
  });

  it("surfaces an Apex failure as a visible error", async () => {
    getDescendants.mockRejectedValue({
