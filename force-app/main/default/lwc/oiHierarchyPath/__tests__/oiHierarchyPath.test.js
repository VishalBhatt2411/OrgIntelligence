...
jest.mock(
  "@salesforce/apex/OI_HierarchyQueryController.getPath",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
...
function setAllProps(
  element,
  { definitionId = "def1", objectApiName = "Account", recordId = "001x3" } = {}
) {
...
describe("c-oi-hierarchy-path", () => {
  ...
  it("renders a root-first trail plus a non-clickable current-record crumb derived from the last entry's own resolved childLabel", async () => {
    getPath.mockResolvedValue([
      {
        relationshipId: "r1",
        parentRecordId: "001x1",
        parentObjectApiName: "Account",
        parentLabel: "Global Enterprise",
        childRecordId: "001x2",
        childObjectApiName: "Account",
        childLabel: "North America"
      },
      {
        relationshipId: "r2",
        parentRecordId: "001x2",
        parentObjectApiName: "Account",
        parentLabel: "North America",
        childRecordId: "001x3",
        childObjectApiName: "Account",
        childLabel: "ABC Motors"
      }
    ]);
    const element = createElement("c-oi-hierarchy-path", {
      is: OiHierarchyPath
    });
    setAllProps(element);
  ...
  it("re-fetches when recordId changes on the same instance (e.g. Record Page navigation)", async () => {
    ...
    expect(getPath).toHaveBeenLastCalledWith({
      definitionId: "def1",
      childObjectApiName: "Account",
      childRecordId: "001x9"
    });
  ...
  it("reports a root-level record honestly instead of fabricating a current-record label", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="path-root-note"]')
    ).not.toBeNull();
    expect(
      element.shadowRoot.querySelector('[data-id="path-crumb-current"]')
    ).toBeNull();
  });
  ...
  it("dispatches recordselect with the clicked ancestor's objectApiName/recordId, never navigating on its own", async () => {
    ...
    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({
        detail: { objectApiName: "Account", recordId: "001x1" }
      })
    );
  });
  ...
  it("broadcasts the current record's resolved label so a sibling component can reuse it without its own query", async () => {
    ...
    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({ detail: { label: "ABC Motors" } })
    );
  });
  ...
  it("surfaces an Apex failure as a visible error", async () => {
    getPath.mockRejectedValue({
      body: { message: "Unknown hierarchy definition." }
    });
    const element = createElement("c-oi-hierarchy-path", {
      is: OiHierarchyPath
    });
    ...
    expect(error.message).toBe("Unknown hierarchy definition.");
  });
});
