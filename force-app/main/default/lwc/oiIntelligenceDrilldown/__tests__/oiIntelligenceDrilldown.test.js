...
jest.mock(
  "@salesforce/apex/OI_GraphController.getNodeConnections",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_GraphController.getNavigationTarget",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
...
describe("c-oi-intelligence-drilldown", () => {
  ...
  function createDrilldown(props) {
    const element = createElement("c-oi-intelligence-drilldown", {
      is: OiIntelligenceDrilldown
    });
  ...
  it("loads and renders the first page on connect, with an accurate header and result count", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-title"]')
        .textContent
    ).toBe("Account");
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-count"]')
        .textContent
    ).toContain("1 result");
    expect(
      element.shadowRoot.querySelectorAll('[data-id="drilldown-row"]')
    ).toHaveLength(1);
  });
  ...
  it("shows a loading state while the first page is in flight", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-loading"]')
    ).not.toBeNull();
  ...
  it("renders a sanitized error state when the fetch fails, and Retry re-issues the request", async () => {
    getNodeConnections.mockRejectedValueOnce({
    ...
    const banner = element.shadowRoot.querySelector(
      '[data-id="drilldown-error"]'
    );
    expect(banner.state).toBe("error");
    expect(banner.message).toContain("permission");

    getNodeConnections.mockResolvedValueOnce(page());
    banner.dispatchEvent(new CustomEvent("retry"));
    await flushPromises();

    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-error"]')
    ).toBeNull();
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-table"]')
    ).not.toBeNull();
  });
  ...
  it("renders an honest empty state distinct from an error, when there are genuinely no results", async () => {
    ...
    const banner = element.shadowRoot.querySelector(
      '[data-id="drilldown-empty"]'
    );
    expect(banner.state).toBe("true-zero");
    expect(banner.message).toContain("No connections of this type");
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-error"]')
    ).toBeNull();
  });
  ...
  it("renders the unavailable reason instead of an empty table when the category is unsupported", async () => {
    getNodeConnections.mockResolvedValue(
      page({
        rows: [],
        totalCount: 0,
        unavailableReason: "Field-level grants are not scanned yet."
      })
    );
    const element = createDrilldown();
    await flushPromises();
    ...
    const banner = element.shadowRoot.querySelector(
      '[data-id="drilldown-unavailable"]'
    );
    expect(banner.state).toBe("coverage-incomplete");
    expect(banner.message).toBe("Field-level grants are not scanned yet.");
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-table"]')
    ).toBeNull();
  });
  ...
  it("debounces search input and re-queries the server with the term, restarting pagination", async () => {
    ...
    getNodeConnections.mockResolvedValue(
      page({ rows: [row({ label: "Owner" })], totalCount: 1, isFiltered: true })
    );

    jest.useFakeTimers();
    const searchInput = element.shadowRoot.querySelector(
      '[data-id="drilldown-search"]'
    );
    ...
    expect(getNodeConnections).toHaveBeenCalledWith(
      expect.objectContaining({ searchTerm: "Owner", cursor: null })
    );
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-count"]')
        .textContent
  ...
  it("sorts loaded rows client-side by name ascending by default, then toggles to descending on a repeat click of the same column", async () => {
    getNodeConnections.mockResolvedValue(
      page({
        rows: [
          row({ nodeKey: "b", label: "Bravo" }),
          row({ nodeKey: "a", label: "Alpha" })
        ],
        totalCount: 2
      })
    );
  ...
  it('loads the next page on "Load more" and appends without duplicating rows', async () => {
    getNodeConnections.mockResolvedValueOnce(
      page({
        rows: [row({ nodeKey: "a" })],
        totalCount: 2,
        hasMore: true,
    ...
    await flushPromises();
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-load-more"]')
    ).not.toBeNull();
    ...
    expect(getNodeConnections).toHaveBeenLastCalledWith(
  ...
  it('resolves navigation and opens the target when "Open in Setup" is clicked', async () => {
    getNodeConnections.mockResolvedValue(page());
    getNavigationTarget.mockResolvedValue({
      kind: "setupPage",
      url: "/lightning/setup/ObjectManager/Account/FieldsAndRelationships/Owner/view"
    });
    const element = createDrilldown();
    await flushPromises();
    ...
    expect(getNavigationTarget).toHaveBeenCalledWith({
      typeKey: "SalesforceMetadata.CustomField",
      apiName: "Account.OwnerId"
    });
  ...
  it("surfaces an honest message when a row cannot be opened directly", async () => {
    ...
    getNavigationTarget.mockResolvedValue({
      kind: "unsupported",
      reason: "This component cannot be opened directly."
    });
    navigateToTarget.mockReturnValueOnce({
      navigated: false,
      message: "This component cannot be opened directly."
    });
    const element = createDrilldown();
    await flushPromises();
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="drilldown-action-message"]')
        .textContent
    ).toBe("This component cannot be opened directly.");
  });
  ...
  it("dispatches close on Escape for keyboard users", async () => {
    ...
    element.shadowRoot
      .querySelector('[data-id="drilldown"]')
      .dispatchEvent(
