...
export default class OiIntelligenceDrilldown extends NavigationMixin(
  ...
  get resultSummary() {
    ...
    const base = this.isFiltered
      ? `${this.totalCount} matching ${noun}`
      : `${this.totalCount} ${noun}`;
  ...
  get sortScopeNote() {
    return this.hasMore
      ? "Sorting applies to the results loaded so far."
      : null;
  }

  /**
   * Both branches are a genuine `true-zero` (ADR-0028 §7): a filtered zero is the confirmed
   * answer to the filtered question, not missing coverage. The title says which question was
   * answered so the state never reads as "nothing exists" when a search is narrowing it.
   */
  get emptyStateTitle() {
    return this.isFiltered ? "No matches" : "No connections";
  }

  get emptyStateMessage() {
    return this.isFiltered
      ? "No results match your search."
      : "No connections of this type were found in the scanned data.";
  }

  /** The error banner's Retry — re-issues exactly the request that failed. */
  handleRetry() {
  ...
  handleRowSelect(event) {
    ...
    if (row) {
      this.dispatchEvent(
        new CustomEvent("nodeselect", { detail: { nodeKey: row.nodeKey } })
      );
    }
  }
