...
jest.mock(
  "@salesforce/apex/OI_HierarchyDefinitionController.getDefinitions",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyDefinitionController.saveDefinition",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyDefinitionController.getLevels",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyDefinitionController.saveLevel",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyDefinitionController.deleteLevel",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyRelationshipController.getRelationships",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyRelationshipController.assignParent",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyRelationshipController.deactivateRelationship",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/OI_HierarchyRelationshipController.getHistory",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
...
describe("c-oi-hierarchy-manager", () => {
  ...
  it("states a confirmed zero for the definitions list rather than rendering an empty void", async () => {
    ...
    const empty = element.shadowRoot.querySelector(
      '[data-id="definitions-empty"]'
    );
    expect(empty).not.toBeNull();
    expect(empty.state).toBe("true-zero");
    expect(
      element.shadowRoot.querySelector('[data-id="definitions-list"]')
    ).toBeNull();
  });
  ...
  it("lists loaded definitions and hides the Levels section until an existing one is selected", async () => {
    ...
    const rows = element.shadowRoot.querySelectorAll(
      '[data-id="definition-row"]'
    );
    expect(rows).toHaveLength(1);
    expect(
      element.shadowRoot.querySelector('[data-id="add-level-button"]')
    ).toBeNull();
  });
    ...
    getLevels.mockResolvedValue([
      {
        ...
        objectApiName: "Account",
        isRequired: true,
        relationshipFieldApiName: "ParentId"
      }
    ]);
    const element = createElement("c-oi-hierarchy-manager", {
      is: OiHierarchyManager
  ...
  it("reveals the Levels section immediately after saving a brand-new definition, without a page reload", async () => {
    ...
    element.shadowRoot
      .querySelector('[data-id="save-definition-button"]')
      .click();
    await flushPromises();
  ...
  it("removes a deleted Level from the local list on delete", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="levels-empty"]')
    ).not.toBeNull();
  });

  it("surfaces a save failure as a visible form error, never a silently dropped save", async () => {
    getDefinitions.mockResolvedValue([]);
    saveDefinition.mockRejectedValue({
      body: {
        message:
          "Duplicate Relationship — this hierarchy relationship already exists."
      }
    });
    const element = createElement("c-oi-hierarchy-manager", {
      is: OiHierarchyManager
  ...
  it("shows an empty state when a definition has no relationships yet", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="relationships-empty"]')
    ).not.toBeNull();
  });
  ...
  it("assigns a new relationship by picking child/parent records via oiRecordPicker and saving", async () => {
    ...
    await flushPromises();
    element.shadowRoot
      .querySelector('[data-id="add-relationship-button"]')
      .click();
    await flushPromises();

    element.shadowRoot
      .querySelector('[data-id="child-record-picker"]')
      .dispatchEvent(
        new CustomEvent("select", {
          detail: { recordId: "001x1", label: "Acme Corp" }
        })
      );
    await flushPromises();
    element.shadowRoot
      .querySelector('[data-id="parent-record-picker"]')
      .dispatchEvent(
        new CustomEvent("select", {
          detail: { recordId: "001root", label: "Root Co" }
        })
      );
    await flushPromises();
    element.shadowRoot
      .querySelector('[data-id="save-relationship-button"]')
      .click();
    await flushPromises();
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="relationship-form"]')
    ).toBeNull();
    const table = element.shadowRoot.querySelector(
      '[data-id="relationships-table"]'
    );
  ...
  it("deactivates a relationship", async () => {
    ...
    await flushPromises();
    element.shadowRoot
      .querySelector('[data-id="deactivate-relationship-button"]')
      .click();
    await flushPromises();

    expect(deactivateRelationship).toHaveBeenCalledWith({
      relationshipId: "rel1"
    });
    expect(
      element.shadowRoot.querySelector('[data-id="relationships-table"]')
        .textContent
    ).toContain("Inactive");
  });
  ...
  it("views a relationship's history and closes it again on a second click of the same row", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="history-list"]').textContent
    ).toContain("Created");
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="history-panel"]')
    ).toBeNull();
  });
  ...
  it("surfaces a relationship save failure as a visible form error", async () => {
    ...
    getLevels.mockResolvedValue([]);
    getRelationships.mockResolvedValue([]);
    assignParent.mockRejectedValue({
      body: {
        message:
          "Invalid Parent — the selected hierarchy level is not valid for this hierarchy."
      }
    });
    const element = createElement("c-oi-hierarchy-manager", {
      is: OiHierarchyManager
    ...
    const error = element.shadowRoot.querySelector(
      '[data-id="relationship-form-error"]'
    );
    expect(error.state).toBe("error");
    expect(error.message).toBe(
      "Invalid Parent — the selected hierarchy level is not valid for this hierarchy."
  ...
  it("a move (single-parent Definition) locally deactivates the sibling relationship for the same child, without re-fetching the cacheable list", async () => {
    getDefinitions.mockResolvedValue([EXISTING_DEFINITION]);
    getLevels.mockResolvedValue([]);
    const existingActiveParent = {
      ...EXISTING_RELATIONSHIP,
      relationshipId: "rel1",
      parentObjectApiName: "Account",
      parentRecordId: "001oldparent",
      parentLabel: "Old Parent"
    };
    getRelationships.mockResolvedValue([existingActiveParent]);
    const movedRelationship = {
      ...EXISTING_RELATIONSHIP,
      relationshipId: "rel2",
      parentObjectApiName: "Account",
      parentRecordId: "001newparent",
    ...
    element.shadowRoot
      ...
      .dispatchEvent(
        new CustomEvent("select", {
          detail: { recordId: "001newparent", label: "New Parent" }
        })
      );
    await flushPromises();
    element.shadowRoot
    ...
    const rows = Array.from(
      element.shadowRoot.querySelectorAll(
        '[data-id="relationships-table"] tbody tr'
      )
    );
    const oldParentRow = rows.find((row) =>
      row.textContent.includes("Old Parent")
    );
    const newParentRow = rows.find((row) =>
      row.textContent.includes("New Parent")
    );
  ...
  /*
   * The history query's arguments arrive on a click event that is gone by the time the error
   * banner's Retry is pressed, so this is the one retry path that needed the ids held in state.
   */
  it("re-issues the history query with the same arguments when the history error banner asks to retry", async () => {
    getDefinitions.mockResolvedValue([EXISTING_DEFINITION]);
    getLevels.mockResolvedValue([]);
    getRelationships.mockResolvedValue([EXISTING_RELATIONSHIP]);
    ...
    const error = element.shadowRoot.querySelector('[data-id="history-error"]');
    expect(error.state).toBe("error");
    expect(error.message).toBe("History unavailable.");

    getHistory.mockResolvedValue([]);
    error.dispatchEvent(new CustomEvent("retry"));
    await flushPromises();
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="history-empty"]').state
    ).toBe("true-zero");
  });
});
