...
export default class OiHierarchyManager extends LightningElement {
  ...
  historyChildObjectApiName = null;
  historyChildRecordId = null;
  ...
  async loadDefinitions() {
    ...
    try {
      ...
      this.definitions = [];
      this.definitionsError = errorMessageFor(
        error,
        "Something went wrong loading hierarchy definitions. Please try again."
      );
    } finally {
  ...
  /**
   * The levels and relationships lists each state their own confirmed zero; the definitions
   * list did not, so an org with no hierarchies yet rendered an empty void under the
   * "Hierarchies" heading — indistinguishable from a list that had failed to load.
   */
  get hasDefinitions() {
  ...
  handleAllowMultipleParentsChange(event) {
    this.definitionDraft = {
      ...this.definitionDraft,
      allowMultipleParents: event.target.checked
    };
  }
  ...
  handleAllowMultipleHierarchiesChange(event) {
    this.definitionDraft = {
      ...this.definitionDraft,
      allowMultipleHierarchies: event.target.checked
    };
  }
  ...
  async handleSaveDefinition() {
    ...
    try {
      ...
      const saved = await saveDefinition({
        ...
        maxLevels:
          draft.maxLevels === "" ||
          draft.maxLevels === null ||
          draft.maxLevels === undefined
            ? null
            : Number(draft.maxLevels),
      ...
      this.definitionFormError = errorMessageFor(
        error,
        "Something went wrong saving the hierarchy. Please try again."
      );
    } finally {
  ...
  upsertDefinitionLocally(saved) {
    const index = this.definitions.findIndex(
      (candidate) => candidate.definitionId === saved.definitionId
    );
    if (index === -1) {
  ...
  async loadLevels(definitionId) {
    ...
    try {
      ...
      this.levels = [];
      this.levelsError = errorMessageFor(
        error,
        "Something went wrong loading hierarchy levels. Please try again."
      );
    } finally {
  ...
  get levelRows() {
    return this.levels.map((level) => ({
      ...level,
      requiredLabel: level.isRequired ? "Yes" : "No"
    }));
  }
  ...
  async handleSaveLevel() {
    ...
    try {
      ...
      const saved = await saveLevel({
        ...
        levelNumber:
          draft.levelNumber === "" ||
          draft.levelNumber === null ||
          draft.levelNumber === undefined
            ? null
            : Number(draft.levelNumber),
        objectApiName: draft.objectApiName,
      ...
      this.levelFormError = errorMessageFor(
        error,
        "Something went wrong saving the hierarchy level. Please try again."
      );
    } finally {
  ...
  upsertLevelLocally(saved) {
    const index = this.levels.findIndex(
      (candidate) => candidate.levelId === saved.levelId
    );
    if (index === -1) {
  ...
  async handleDeleteLevel(event) {
    ...
    try {
      ...
      this.levelsError = errorMessageFor(
        error,
        "Something went wrong deleting the hierarchy level. Please try again."
      );
    }
  }
  ...
  async loadRelationships(definitionId) {
    ...
    try {
      ...
      this.relationships = [];
      this.relationshipsError = errorMessageFor(
        error,
        "Something went wrong loading hierarchy relationships. Please try again."
      );
    } finally {
  ...
  get relationshipRows() {
    return this.relationships.map((relationship) => ({
      ...relationship,
      parentDisplay: relationship.parentRecordId
        ? relationship.parentLabel || relationship.parentRecordId
        : "(root)",
      childDisplay: relationship.childLabel || relationship.childRecordId,
      statusLabel: relationship.isActive ? "Active" : "Inactive",
      isHistoryOpenForThisRow:
        relationship.relationshipId === this.historyForRelationshipId,
  ...
  handleParentSelect(event) {
    this.relationshipDraft = {
      ...this.relationshipDraft,
      parentRecordId: event.detail.recordId,
      parentLabel: event.detail.label
    };
  }
  ...
  handleChildSelect(event) {
    this.relationshipDraft = {
      ...this.relationshipDraft,
      childRecordId: event.detail.recordId,
      childLabel: event.detail.label
    };
  }
  ...
  handleNewRelationshipClick() {
    this.relationshipDraft = blankRelationshipDraft(
      this.definitionDraft.definitionId
    );
    this.relationshipFormError = null;
  }
  ...
  async handleSaveRelationship() {
    ...
    try {
      ...
      this.relationshipFormError = errorMessageFor(
        error,
        "Something went wrong saving the hierarchy relationship. Please try again."
      );
    } finally {
  ...
  upsertRelationshipLocally(saved) {
    ...
    const next = this.relationships
      .filter(
        (relationship) => relationship.relationshipId !== saved.relationshipId
      )
      .map((relationship) => {
        const shouldDeactivate =
          deactivateSiblings &&
          relationship.isActive &&
          relationship.childRecordId === saved.childRecordId;
  ...
  async handleDeactivateRelationship(event) {
    ...
    try {
      ...
      this.relationshipsError = errorMessageFor(
        error,
        "Something went wrong deactivating the hierarchy relationship. Please try again."
      );
    }
  }
  ...
  handleViewHistoryClick(event) {
    ...
    /* Held so the error banner's Retry can re-issue the same query — the click event that carried them is long gone by then. */
    this.historyChildObjectApiName =
      event.currentTarget.dataset.childObjectApiName;
    this.historyChildRecordId = event.currentTarget.dataset.childRecordId;
    this.loadHistory();
  }
  ...
  async loadHistory() {
    ...
    this.historyError = null;
    try {
      this.historyRows = await getHistory({
        definitionId: this.definitionDraft.definitionId,
        childObjectApiName: this.historyChildObjectApiName,
        childRecordId: this.historyChildRecordId
      });
    } catch (error) {
      this.historyRows = [];
      this.historyError = errorMessageFor(
        error,
        "Something went wrong loading hierarchy history. Please try again."
      );
    } finally {
  ...
  handleCloseHistory() {
    this.historyForRelationshipId = null;
    this.historyChildObjectApiName = null;
    this.historyChildRecordId = null;
    this.historyRows = [];
    this.historyError = null;
  }

  /*
   * Retry handlers for the four load failures. Each simply re-issues the same query with inputs
   * already held in state; the three SAVE failures wire Retry straight to their own save handler,
   * which is idempotent in the same way. c/oiStateBanner renders Retry for every error state by
   * construction, so leaving any of these unwired would have shipped a dead button.
   */
  handleRetryDefinitions() {
    this.loadDefinitions();
  }

  handleRetryLevels() {
    this.loadLevels(this.definitionDraft.definitionId);
  }

  handleRetryRelationships() {
    this.loadRelationships(this.definitionDraft.definitionId);
  }
