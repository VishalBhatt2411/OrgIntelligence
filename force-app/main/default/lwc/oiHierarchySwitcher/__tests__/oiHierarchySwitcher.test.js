...
describe("c-oi-hierarchy-switcher", () => {
  ...
  it("narrows to only Active definitions scoped to the given objectApiName and auto-selects the first one", async () => {
    ...
    const element = createElement("c-oi-hierarchy-switcher", {
      is: OiHierarchySwitcher
    });
  ...
  it("shows an honest empty state when no Active definition applies to the object, never a broken dropdown", async () => {
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="switcher-empty"]')
    ).not.toBeNull();
    expect(
      element.shadowRoot.querySelector('[data-id="switcher-combobox"]')
    ).toBeNull();
  });
  ...
  it("re-derives the selection when objectApiName changes to a different object on the same instance", async () => {
    ...
    expect(handler).toHaveBeenCalledWith(
      expect.objectContaining({ detail: { definitionId: "def4" } })
    );
  });
  ...
  /* c/oiStateBanner renders a Retry button for every error state, so an unwired onretry ships a dead control. */
  it("re-issues the fetch when the error banner asks to retry", async () => {
    getDefinitions.mockRejectedValueOnce({
    ...
    getDefinitions.mockResolvedValue(DEFINITIONS);
    element.shadowRoot
      .querySelector('[data-id="switcher-error"]')
      .dispatchEvent(new CustomEvent("retry"));
    await flushPromises();

    expect(getDefinitions).toHaveBeenCalledTimes(2);
    expect(
      element.shadowRoot.querySelector('[data-id="switcher-error"]')
    ).toBeNull();
    expect(
      element.shadowRoot.querySelector('[data-id="switcher-combobox"]')
    ).not.toBeNull();
