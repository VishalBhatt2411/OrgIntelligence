...
describe("c-oi-schema-object-card", () => {
  ...
  it("dispatches select with the nodeKey when the header is clicked", () => {
    const element = createElement("c-oi-schema-object-card", {
      is: OiSchemaObjectCard
    });
    element.nodeKey = "n1";
  ...
  it("renders a plain field-count line instead of individual field rows — field browsing lives in the Detail Panel now, not on the canvas card", () => {
    ...
    return Promise.resolve().then(() => {
      expect(
        element.shadowRoot.querySelectorAll('[data-id="field-row"]')
      ).toHaveLength(0);
      expect(
        element.shadowRoot.querySelector('[data-id="fields-toggle"]')
      ).toBeNull();
      expect(
        element.shadowRoot.querySelector('[data-id="field-count"]').textContent
      ).toBe("12 fields");
    });
  });
  ...
  it("resolves a known registry colorToken to a real CSS color for the accent border, not the raw token name", () => {
    ...
    return Promise.resolve().then(() => {
      const style = element.shadowRoot
        .querySelector('[data-id="schema-card"]')
        .getAttribute("style");
  ...
  it("applies the is-selected class when isSelected is true", () => {
    ...
    return Promise.resolve().then(() => {
      expect(
        element.shadowRoot.querySelector('[data-id="schema-card"]').className
