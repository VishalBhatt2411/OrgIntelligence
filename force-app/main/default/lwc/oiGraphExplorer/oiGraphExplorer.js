...
} from "c/graphRelationshipFilter";
...
const OBJECT_ICON_URL_PATTERN =
  /\/(standard|custom)\/([a-zA-Z0-9_]+?)(?:_\d+)?\.(?:png|svg)(?:[?#].*)?$/i;
...
export default class OiGraphExplorer extends NavigationMixin(LightningElement) {
  ...
  /**
   * The operation the error banner's Retry re-runs. Every fetch the shell performs sets this to
   * a re-invocation of itself before it starts, so the one banner can retry whichever of the
   * five entry points (center, expand, record center, field-list, record-object) actually
   * failed. Errors raised without a fetch behind them (a node that cannot be opened, a
   * navigation the platform refused) leave it null — see handleErrorRetry.
   */
  retryOperation = null;
  ...
  get allCanvasNodes() {
    ...
    const value = snapshotNodes(this.viewState).map((node) => {
      ...
      const perObjectIconName =
        node.typeKey === OBJECT_TYPE_KEY
          ? this.objectIconByApiName.get(node.secondaryKey)
          : null;
      return {
        ...node,
    ...
    this._allCanvasNodesCache = {
      ...
      objectIconByApiName: this.objectIconByApiName,
      value
    };
    return value;
  }
  ...
  get allCanvasEdges() {
    if (
      this._allCanvasEdgesCache &&
      this._allCanvasEdgesCache.viewState === this.viewState &&
      this._allCanvasEdgesCache.registry === this.registry
    ) {
  ...
  get relationshipFilter() {
    return {
      ...createRelationshipFilter(),
      edgeTypeKeys: this.allowedEdgeTypeKeys,
      direction: this.relationshipDirection,
      maxDepth: this.restrictToDirectOnly ? 1 : null
    };
  }
  ...
  get relationshipOnlyView() {
    ...
    const relationshipFieldKeys = new Set(
      this.allCanvasEdges
        .filter((edge) => edge.typeKey !== HAS_FIELD_EDGE_TYPE_KEY)
    ...
    const prunedFieldKeys = new Set(
      this.allCanvasNodes
        .filter(
          (node) =>
            node.typeKey === FIELD_TYPE_KEY &&
            !relationshipFieldKeys.has(node.nodeKey)
        )
    ...
    return {
      nodes: this.allCanvasNodes.filter(
        (node) => !prunedFieldKeys.has(node.nodeKey)
      ),
      edges: this.allCanvasEdges.filter(
        (edge) =>
          !prunedFieldKeys.has(edge.sourceNodeKey) &&
          !prunedFieldKeys.has(edge.targetNodeKey)
      )
    };
  }
  ...
  get filteredView() {
    ...
    return applyRelationshipFilter(
      nodes,
      edges,
      this.viewState.centerNodeKey,
    ...
    for (const edge of this.allCanvasEdges) {
      if (!seen.has(edge.typeKey)) {
        seen.set(edge.typeKey, {
          ...
          swatchClass:
            "oi-filter-panel-swatch oi-filter-panel-swatch-" + edge.lineStyle
        });
      }
    }
  ...
  get searchPlaceholder() {
    return this.isFieldMode
      ? "First, search for the object this field belongs to (e.g. Account)..."
      : "Search objects (e.g., Account, Opportunity...)";
  }
  ...
  get centerObjectSummary() {
    ...
    return (
      this.allCanvasNodes.find(
        (node) => node.nodeKey === this.viewState.centerNodeKey
      ) || null
    );
  }
  ...
  async handleFieldModeObjectSelect(event) {
    ...
    this.retryOperation = () => this.handleFieldModeObjectSelect(event);
    this.isLoadingFieldOptions = true;
    this.errorMessage = null;
    try {
      const fragment = await getGraphFragment({
        nodeKey: objectNodeKey,
        hopDepth: 1,
        pageCursor: null,
        knownChecksums: {}
      });
      ...
      const objectSummary = (fragment.nodes || []).find(
        (n) => n.nodeKey === objectNodeKey
      );
      /**
      ...
      const ownFieldKeys = new Set(
        (fragment.edges || [])
          .filter(
            (e) =>
              e.typeKey === HAS_FIELD_EDGE_TYPE_KEY &&
              e.sourceNodeKey === objectNodeKey
          )
          .map((e) => e.targetNodeKey)
      );
      const fieldOptions = (fragment.nodes || [])
        .filter(
          (n) => n.typeKey === FIELD_TYPE_KEY && ownFieldKeys.has(n.nodeKey)
        )
        .map((n) => ({
          label: `${n.label} (${n.secondaryKey})`,
          value: n.nodeKey
        }))
        .sort((a, b) => a.label.localeCompare(b.label));
      this.fieldModeObjectKey = objectNodeKey;
      this.fieldModeObjectLabel = objectSummary
        ? objectSummary.label
        : objectNodeKey;
  ...
  async handleRecordModeObjectSelect(event) {
    ...
    const objectNodeKey = event.detail.nodeKey;
    this.retryOperation = () => this.handleRecordModeObjectSelect(event);
  ...
  handleRecordPicked(event) {
    return this.selectAndCenterRecord(
      this.recordModeObjectApiName,
  ...
  async selectAndCenterRecord(objectApiName, recordId) {
    this.retryOperation = () =>
      this.selectAndCenterRecord(objectApiName, recordId);
  ...
  async handleCanvasOpen(event) {
    ...
    if (recordRef) {
      navigateToTarget(this, {
        kind: NAVIGATION_KIND_RECORD,
        recordId: recordRef.recordId,
        objectApiName: recordRef.objectApiName
      });
      return;
    }
    const node = this.allCanvasNodes.find(
    ...
    this.retryOperation = () => this.handleCanvasOpen(event);
    try {
      const target = await getNavigationTarget({
        typeKey: node.typeKey,
        apiName: node.secondaryKey
      });
  ...
  handleHighlightImpact(event) {
    ...
    this.workingSetCeilingHit = isWorkingSetCeilingHit(
      this.viewState,
      WORKING_SET_CEILING
    );
    this.refreshViewState();
  ...
  async selectAndCenter(nodeKey) {
    this.retryOperation = () => this.selectAndCenter(nodeKey);
    const requestId = ++this.centerRequestId;
    this.isLoadingFragment = true;
    this.errorMessage = null;
    try {
  ...
  async handleExpand(event) {
    ...
    this.retryOperation = () => this.handleExpand(event);
    this.isLoadingFragment = true;
    this.errorMessage = null;
    try {
      const fragment = await getGraphFragment({
  ...
  /**
   * Re-runs whatever failed. `retryOperation` is null only for the errors that have no fetch
   * behind them, where retrying could not change the outcome; clearing the message is then the
   * honest response — the banner goes away and the user's graph is untouched.
   */
  handleErrorRetry() {
