...
jest.mock(
  "@salesforce/apex/OI_GraphController.getGraphFragment",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
...
jest.mock(
  "@salesforce/apex/OI_SettingsController.getPresentationRegistry",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
 ...
 */
async function switchToFieldModeAndCenterOnField(
  element,
  { objectFragment, fieldCenterFragment }
) {
  ...
  const picker = element.shadowRoot.querySelector(
    '[data-id="field-mode-field-picker"]'
  );
  picker.dispatchEvent(
    new CustomEvent("change", {
      detail: { value: fieldCenterFragment.centerNodeKey }
    })
  );
  await flushPromises();
}
...
describe("c-oi-graph-explorer", () => {
  ...
  it("shows the placeholder until a node is selected — no fetch happens on initial render (GraphUI.md §14)", async () => {
    const element = createElement("c-oi-graph-explorer", {
      is: OiGraphExplorer
    });
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="explorer-placeholder"]')
    ).not.toBeNull();
  ...
  it("switching Analyze mode tabs clears the previous tab's loaded graph instead of leaving it rendered underneath the new tab (audit #01 — a tab switch is a fresh view, GraphUI.md §11)", async () => {
    getGraphFragment.mockResolvedValueOnce({
      centerNodeKey: "root",
      nodes: [nodeSummaryDto("root", "Root")],
      edges: [],
      frontier: ["root"],
      hasMore: false,
      nextCursor: null
    ...
    element.shadowRoot
      .querySelector("c-oi-search-bar")
      .dispatchEvent(
        new CustomEvent("select", { detail: { nodeKey: "root" } })
      );
    await flushPromises();

    expect(
      element.shadowRoot.querySelector("c-oi-relationship-canvas")
    ).not.toBeNull();
    expect(
      element.shadowRoot.querySelector('[data-id="explorer-placeholder"]')
    ).toBeNull();
  ...
  describe("Object analyze mode — GraphUI.md §42, ADR-0023", () => {
    it("a search selection in the default Object Analyze mode issues a 2-hop getGraphFragment call and renders the Object Relationship canvas, never the radial canvas", async () => {
      getGraphFragment.mockResolvedValue({
        centerNodeKey: "root",
        nodes: [nodeSummaryDto("root", "Root")],
        edges: [],
        frontier: ["root"],
      ...
      expect(getGraphFragment).toHaveBeenCalledWith({
        nodeKey: "root",
        hopDepth: 2,
        pageCursor: null,
        knownChecksums: {}
    ...
    it("passes the full, unfiltered working set to the Object Relationship canvas — the container does no Object-only/field-pruning itself; that filtering is the new canvas's own job (objectRelationshipView.js), not oiGraphExplorer's", async () => {
      getGraphFragment.mockResolvedValueOnce({
        ...
        nodes: [
          nodeSummaryDto("root", "Root"),
          {
            ...
            label: "Description__c",
            secondaryKey: "Root.Description__c",
            state: "Active"
          },
          {
            nodeKey: "apexClass1",
      ...
      expect(canvas.nodes.map((n) => n.nodeKey).sort()).toEqual([
        "apexClass1",
        "plainField",
        "root"
      ]);
      ...
      getGraphFragment.mockResolvedValueOnce({
        ...
        nodes: [
          ...
          nodeSummaryDto("user", "User"),
          {
            ...
            typeKey: "SalesforceMetadata.CustomField",
            label: "AccountId",
            secondaryKey: "Opportunity.AccountId",
            state: "Active"
          },
          {
            nodeKey: "ownerId",
        ...
        edges: [
          {
            edgeKey: "e1",
            typeKey: "SalesforceMetadata.HAS_FIELD",
            sourceNodeKey: "opportunity",
            targetNodeKey: "accountId"
          },
          {
            edgeKey: "e2",
            typeKey: "SalesforceMetadata.LOOKUP_TO",
            sourceNodeKey: "accountId",
          ...
          },
          {
            ...
            targetNodeKey: "ownerId"
          },
          {
            edgeKey: "e4",
            typeKey: "SalesforceMetadata.LOOKUP_TO",
            sourceNodeKey: "ownerId",
            targetNodeKey: "user"
          },
          {
            edgeKey: "e5",
            typeKey: "SalesforceMetadata.HAS_FIELD",
            sourceNodeKey: "account",
            targetNodeKey: "parentId"
          },
          {
            edgeKey: "e6",
            typeKey: "SalesforceMetadata.LOOKUP_TO",
            sourceNodeKey: "parentId",
            targetNodeKey: "account"
          }
        ],
        frontier: ["account"],
    ...
    it('"Explore From Here" re-centers the graph on the neighbor object exactly like a fresh search selection, using Object mode\'s own 2-hop depth', async () => {
      ...
      getGraphFragment.mockResolvedValueOnce({
        centerNodeKey: "neighbor",
        nodes: [nodeSummaryDto("neighbor", "Neighbor")],
        edges: [],
        frontier: ["neighbor"],
        hasMore: false,
        nextCursor: null
      ...
      element.shadowRoot
        ...
        .dispatchEvent(
          new CustomEvent("explorefromhere", {
            detail: { nodeKey: "neighbor" }
          })
        );
      await flushPromises();

      expect(getGraphFragment).toHaveBeenLastCalledWith({
        nodeKey: "neighbor",
        hopDepth: 2,
        pageCursor: null,
        knownChecksums: {}
    ...
    it("an edgeclick from the Object Relationship canvas opens oiRelationshipConnectorDetail with the connector descriptor, and closing it clears the state", async () => {
      ...
      const connector = {
        connectorKey: "outgoing::neighbor",
        direction: "outgoing",
        counterpartObject: { nodeKey: "neighbor", label: "Neighbor" },
        fields: [],
        relationshipCount: 1,
      ...
      const detail = element.shadowRoot.querySelector(
        "c-oi-relationship-connector-detail"
      );
    ...
    it("a highlightimpact event from the detail panel still merges into the shared view-state in Object mode (the reference-counting unification, GraphUI.md §7, is mode-agnostic) even though the merged node is not itself an object-to-object relationship", async () => {
      ...
      const detailPanel = element.shadowRoot.querySelector(
        "c-oi-node-detail-panel"
      );
        ...
        new CustomEvent("highlightimpact", {
          detail: {
            ...
            fragment: {
              nodes: [
                {
                  nodeKey: "impactedClass",
                  typeKey: "SalesforceMetadata.ApexClass",
                  label: "ImpactedClass",
                  secondaryKey: "ImpactedClass",
                  state: "Active"
                }
              ],
              edges: [
                {
                  edgeKey: "impactEdge1",
                  typeKey: "SalesforceMetadata.REFERENCES",
                  sourceNodeKey: "root",
                  targetNodeKey: "impactedClass"
                }
              ],
              hasMore: false,
              nextCursor: null
  ...
  it("Record mode requires picking the object first (zero-hop resolve), then searches actual records of that object and centers on the picked one", async () => {
    getGraphFragment.mockResolvedValueOnce({
      centerNodeKey: "acct",
      nodes: [nodeSummaryDto("acct", "Account")],
      edges: [],
      frontier: ["acct"],
      hasMore: false,
      nextCursor: null
    ...
    expect(
      element.shadowRoot
        .querySelector('[data-id="analyze-mode-record"]')
    ...
    element.shadowRoot
      .querySelector("c-oi-search-bar")
      .dispatchEvent(
        new CustomEvent("select", { detail: { nodeKey: "acct" } })
      );
    await flushPromises();
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="record-mode-object-banner"]')
        .textContent
    ).toContain("Account");
    expect(
      element.shadowRoot.querySelector("c-oi-record-picker").objectApiName
    ).toBe("Account");
    ...
    element.shadowRoot
      .querySelector("c-oi-record-picker")
      .dispatchEvent(
        new CustomEvent("select", { detail: { recordId: "001x1" } })
      );
    await flushPromises();
  ...
  it('"Change object" in Record mode returns to the object search step', async () => {
    ...
    element.shadowRoot
      .querySelector('[data-id="record-mode-change-object"]')
      .click();
    await flushPromises();
  ...
  it('"Explore From Here" on a related record card calls getRecordFragment (via selectAndCenterRecord), not getGraphFragment, based on the nodeKey format alone — Record mode has no expand/collapse of its own, only re-centering (ADR-0024)', async () => {
    ...
    expect(getGraphFragment).not.toHaveBeenCalledWith(
      expect.objectContaining({ nodeKey: "Record::Contact::003x1" })
    );
  });
  ...
  it("Field mode requires picking the object first, then lists only that object's own fields to choose from", async () => {
    getGraphFragment.mockResolvedValueOnce({
      ...
      nodes: [
        ...
        {
          nodeKey: "contact.name",
          typeKey: "SalesforceMetadata.CustomField",
          label: "Contact Name",
          secondaryKey: "Contact.Name",
          state: "Active"
        }
      ],
      edges: [
    ...
    expect(element.shadowRoot.querySelector("c-oi-search-bar")).not.toBeNull();
    expect(
      element.shadowRoot.querySelector('[data-id="field-mode-field-picker"]')
    ).toBeNull();
    ...
    expect(
      element.shadowRoot.querySelector('[data-id="field-mode-object-banner"]')
        .textContent
    ).toContain("Account");
    ...
    getGraphFragment.mockResolvedValueOnce({
      ...
      nodes: [nodeSummaryDto("acct.name", "Account Name")],
      edges: [],
      frontier: ["acct.name"],
      hasMore: false,
      nextCursor: null
    ...
    picker.dispatchEvent(
      new CustomEvent("change", { detail: { value: "acct.name" } })
    );
    await flushPromises();
    ...
    expect(
      element.shadowRoot.querySelector("c-oi-graph-canvas")
    ).not.toBeNull();
    expect(
      element.shadowRoot.querySelector("c-oi-relationship-canvas")
    ).toBeNull();
  ...
  it('"Change object" in Field mode returns to the object search step', async () => {
    ...
    element.shadowRoot
      .querySelector('[data-id="field-mode-change-object"]')
      .click();
    await flushPromises();
  ...
  describe("Field mode regression — the radial canvas + filter panel machinery Object mode no longer exercises directly", () => {
    ...
    it("an expand event issues a one-hop getGraphFragment call and grows the rendered canvas node set", async () => {
        ...
        edges: [
          {
            edgeKey: "e2",
            typeKey: "SalesforceMetadata.HAS_FIELD",
            sourceNodeKey: "acct.name",
            targetNodeKey: "child1"
          }
        ],
        frontier: ["child1"],
        hasMore: false,
      ...
      canvas.dispatchEvent(
        new CustomEvent("expand", { detail: { nodeKey: "acct.name" } })
      );
      await flushPromises();
    ...
    it("passes registry-resolved display labels to the filter panel once edges are visible (Backlog UI-4)", async () => {
      ...
      element.shadowRoot
        .querySelector("c-oi-graph-canvas")
        .dispatchEvent(
          new CustomEvent("expand", { detail: { nodeKey: "acct.name" } })
        );
      await flushPromises();
    ...
    it("unchecking an edge type in the filter panel hides that edge (and any node reachable only through it) from the canvas, and rechecking it restores both", async () => {
      ...
      expect(
        element.shadowRoot.querySelector("c-oi-graph-canvas").edges
      ).toHaveLength(1);
      expect(
    ...
    it('a "Parent Objects" direction selection (outgoing only) hides a node reachable only via an incoming edge from the center', async () => {
      ...
      await switchToFieldModeAndCenterOnField(element, {
        ...
        fieldCenterFragment: {
          ...
          nodes: [
            nodeSummaryDto("contact.name", "Contact Name"),
            nodeSummaryDto("account", "Account"),
            nodeSummaryDto("opportunity", "Opportunity")
          ],
          edges: [
      ...
      filterPanel.dispatchEvent(
        new CustomEvent("directiontoggle", {
          detail: { showParents: true, showChildren: false }
        })
      );
      await flushPromises();
    ...
    it("unchecking both direction checkboxes at once is a no-op — it never leaves the canvas with nothing but the center node", async () => {
      ...
      expect(
        element.shadowRoot.querySelector("c-oi-filter-panel").direction
      ).toBe("both");
    });
      ...
      await switchToFieldModeAndCenterOnField(element, {
        ...
        fieldCenterFragment: {
          ...
          nodes: [
            nodeSummaryDto("contact.name", "Contact Name"),
            nodeSummaryDto("account", "Account"),
            nodeSummaryDto("parentAccount", "Parent Account")
          ],
          edges: [
            {
              edgeKey: "e1",
    ...
    it("selecting a new center resets the relationship filter back to its unrestricted default", async () => {
      ...
      getGraphFragment.mockResolvedValueOnce({
        ...
        nodes: [
          nodeSummaryDto("newRoot", "New Root"),
          nodeSummaryDto("newChild", "New Child")
        ],
        edges: [
          {
            edgeKey: "e2",
            typeKey: "SalesforceMetadata.LOOKUP_TO",
            sourceNodeKey: "newRoot",
            targetNodeKey: "newChild"
          }
        ],
        frontier: [],
        hasMore: false,
      ...
      element.shadowRoot
        .querySelector('[data-id="field-mode-field-picker"]')
        .dispatchEvent(
          new CustomEvent("change", { detail: { value: "newRoot" } })
        );
      await flushPromises();
  ...
  describe("Object mode search hierarchy (VisualDesignSpecification.md §3.1/§3.2 — analyzed-state composition)", () => {
    it('collapses the full search bar into a compact "Object: <label>" banner once an object is analyzed, and restores the search bar via "Change object"', async () => {
      ...
      expect(
        element.shadowRoot.querySelector("c-oi-search-bar")
      ).not.toBeNull();
      expect(
        element.shadowRoot.querySelector('[data-id="object-mode-banner"]')
      ).toBeNull();
      ...
      const banner = element.shadowRoot.querySelector(
        '[data-id="object-mode-banner"]'
      );
      expect(banner).not.toBeNull();
      expect(banner.textContent).toContain("Root");

      element.shadowRoot
        .querySelector('[data-id="object-mode-change-object"]')
        .click();
      await flushPromises();
    ...
    it("re-collapses the search bar back to the compact banner once a new object is selected from it", async () => {
      ...
      getGraphFragment.mockResolvedValueOnce({
        centerNodeKey: "other",
        nodes: [nodeSummaryDto("other", "Other")],
        edges: [],
        frontier: ["other"],
        hasMore: false,
        nextCursor: null
      ...
      element.shadowRoot
        .querySelector("c-oi-search-bar")
        .dispatchEvent(
          new CustomEvent("select", { detail: { nodeKey: "other" } })
        );
      await flushPromises();
  ...
  it("renders a sanitized error banner when getGraphFragment fails, without throwing, and Retry re-issues the request", async () => {
    getGraphFragment.mockRejectedValueOnce({
    ...
    const errorEl = element.shadowRoot.querySelector(
      '[data-id="explorer-error"]'
    );
    expect(errorEl).not.toBeNull();
    expect(errorEl.state).toBe("error");
    expect(errorEl.message).toContain("permission");

    /** Retry must re-issue the request that failed, not merely dismiss the message. */
    getGraphFragment.mockResolvedValueOnce({
      centerNodeKey: "root",
      nodes: [nodeSummaryDto("root", "Root")],
      edges: [],
    ...
    expect(getGraphFragment).toHaveBeenCalledTimes(2);
    expect(
      element.shadowRoot.querySelector('[data-id="explorer-error"]')
    ).toBeNull();
  });
});
