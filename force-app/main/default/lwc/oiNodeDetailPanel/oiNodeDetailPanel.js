...
/**
 * Maps this panel's four internal empty-state *causes* onto ADR-0028 §7's canonical state
 * vocabulary, which is what the platform's shared `c/oiStateBanner` speaks.
 *
 * The four causes survive because they select genuinely different copy (see
 * buildEmptyStateMessage) — the user needs to know whether a zero means "nothing is there",
 * "nothing was scanned", or "what was scanned is old". What does NOT survive is a fifth
 * private *visual* vocabulary: all three "the data behind this zero cannot be trusted" causes
 * resolve to `coverage-incomplete`, and only a scan-current zero resolves to `true-zero`.
 * `not-obtainable` is deliberately unreachable here: none of these four causes is a structural
 * "the platform will never expose this" fact — per-category Unsupported-vs-Degraded precision
 * would need the additive DTO status field noted in intelligenceSections' own doc comment, and
 * claiming it without that field would be a fabricated state.
 */
const CANONICAL_STATE_BY_EMPTY_KIND = {
  zero: "true-zero",
  unscanned: "coverage-incomplete",
...
export default class OiNodeDetailPanel extends LightningElement {
  ...
  async loadIntelligence() {
    ...
    try {
      ...
      this.intelligence = null;
      this.intelligenceErrorMessage =
        (error && error.body && error.body.message) ||
        "Something went wrong loading intelligence for this component.";
    } finally {
      if (requestId === this.intelligenceRequestId) {
        this.isLoadingIntelligence = false;
      }
  ...
  get intelligenceSections() {
    ...
    return this.intelligence.categories.map((category) => {
      ...
      const emptyStateKind = hasItems
        ? null
        : this.classifyEmptyState(category);
      return {
        key: category.category,
        title: category.category,
        iconName:
          CATEGORY_ICON_NAMES[category.category] || "utility:knowledge_base",
        iconModifierClass:
          "oi-node-detail-panel-section-icon is-" +
        ...
        isCoverageDetailsExpanded: this.expandedCoverageDetails.has(
          category.category
        ),
        emptyStateKind,
        emptyState: emptyStateKind
          ? CANONICAL_STATE_BY_EMPTY_KIND[emptyStateKind]
          : null,
        emptyStateMessage: emptyStateKind
          ? buildEmptyStateMessage(category.category, emptyStateKind)
          : null,
        emptyStateBadgeClass: emptyStateKind
          ? `oi-node-detail-panel-empty-state-badge oi-node-detail-panel-empty-state_${CANONICAL_STATE_BY_EMPTY_KIND[emptyStateKind]}`
          : null,
  ...
  /** The panel-wide banner's Retry — re-fetches the detail for the currently selected node. */
  handleDetailRetry() {
    return this.loadDetail();
  }

  /** The intelligence banner's Retry — re-fetches only that payload, leaving the rest of the panel's already-loaded content in place. */
  handleIntelligenceRetry() {
    return this.loadIntelligence();
  }
  ...
  get isIntelligenceStale() {
    ...
    return (
      Date.now() - new Date(this.intelligence.lastScannedAt).getTime() >
      STALE_THRESHOLD_MS
    );
  }
  ...
  get pinToggleLabel() {
    return this.isPanelPinned
      ? "Unpin panel layout (stop keeping sections open across selections)"
      : "Pin panel layout (keep sections open when you select another node)";
  }

  get pinToggleClass() {
    return (
      "oi-node-detail-panel-pin-toggle" +
      (this.isPanelPinned ? " is-active" : "")
    );
  }
  ...
  get technicalDetailsToggleLabel() {
    return this.technicalDetailsVisible
  ...
  async loadDetail() {
    ...
    try {
      ...
      if (recordRef) {
        const fragment = await getRecordFragment({
          objectApiName: recordRef.objectApiName,
          recordId: recordRef.recordId
        });
        if (requestId !== this.detailRequestId) {
          return;
        }
        const summary = (fragment.nodes || []).find(
          (n) => n.nodeKey === requestedNodeKey
        );
        if (!summary) {
          this.detail = null;
      ...
      if (requestId === this.detailRequestId) {
        ...
        this.errorMessage =
          (error && error.body && error.body.message) ||
          "Something went wrong loading node detail.";
      }
    } finally {
      if (requestId === this.detailRequestId) {
  ...
  recordTypeDisplayLabel(typeKey) {
    ...
    return typeKey && typeKey.startsWith(RECORD_TYPE_PREFIX)
      ? typeKey.slice(RECORD_TYPE_PREFIX.length)
      : typeKey;
  }
  ...
  get recordObjectApiName() {
    return this.detail && this.detail.recordRef
      ? this.detail.recordRef.objectApiName
      : "";
  }
  ...
  get typeDisplayLabel() {
    ...
    return (
      resolveNodeStyle(this.registry, this.detail.typeKey).displayLabel ||
      this.detail.typeKey
    );
  }
  ...
  get fieldRelationshipTypeLabel() {
    ...
    const outgoingTypes = Object.keys(
      this.detail.outgoingRelationshipCounts
    ).filter((typeKey) => typeKey !== HAS_FIELD_TYPE_KEY);
    ...
    return outgoingTypes
      .map(
        (typeKey) =>
          resolveEdgeStyle(this.registry, typeKey).displayLabel || typeKey
      )
      .join(", ");
  }
  ...
  get overviewFields() {
    ...
    if (this.isField) {
      fields.push({
        key: "type",
        label: "Type",
        value: this.overviewTypeLabel
      });
      fields.push({
        key: "dataType",
        label: "Data Type",
        value: this.dataTypeDisplay
      });
      if (this.hasCustomFlag) {
        fields.push({
          key: "customStandard",
          label: "Custom / Standard",
          value: this.customOrStandardLabel
        });
      }
  ...
  handleDrilldownOpen(event) {
    ...
    const row = this.relationshipCountRows.find(
  ...
  get curatedRelationshipRows() {
    ...
    const rows = [
      {
        ...
        count: incoming[LOOKUP_TO_TYPE_KEY] || 0,
    ...
    if (this.objectRelationshipSummary) {
      rows.push(
        {
          key: "self",
          label: "Self Relationships",
          count: this.objectRelationshipSummary.selfRelationships,
          isDrilldownEligible: false
        },
        {
          key: "referenced-objects",
          label: "Referenced Objects",
          count: this.objectRelationshipSummary.referencedObjects,
          isDrilldownEligible: false
        },
        {
          key: "referencing-objects",
          label: "Referencing Objects",
          count: this.objectRelationshipSummary.referencingObjects,
          isDrilldownEligible: false
        }
      );
    }
  ...
  get relationshipSectionCount() {
    return this.hasCuratedRelationshipRows
      ? this.curatedRelationshipRows.length
      : this.relationshipCountRows.length;
  }
  ...
  handleCuratedDrilldownOpen(event) {
    const key = event.currentTarget.dataset.rowKey;
    const row = this.curatedRelationshipRows.find(
      (candidate) => candidate.key === key
    );
  ...
  get attributeRows() {
    ...
    return Object.keys(this.detail.attributes)
      ...
      .map((key) => ({
        key,
        value:
          this.detail.attributes[key] === null ||
          this.detail.attributes[key] === undefined
            ? ""
            : String(this.detail.attributes[key])
      }));
  }
  ...
  get fieldCount() {
    return (
      (this.detail &&
        this.detail.outgoingRelationshipCounts &&
        this.detail.outgoingRelationshipCounts[HAS_FIELD_TYPE_KEY]) ||
      0
    );
  }
  ...
  async loadFieldSummaries() {
    ...
    this.fieldsErrorMessage = null;
    try {
      const summaries = await getFieldSummaries({
        objectNodeKey: requestedNodeKey
      });
      ...
      if (requestId === this.fieldRequestId) {
        ...
        this.fieldsErrorMessage =
          (error && error.body && error.body.message) ||
          "Something went wrong loading fields.";
      }
    } finally {
      if (requestId === this.fieldRequestId) {
        this.isLoadingFields = false;
  ...
  get fieldTypeFilterOptions() {
    return FIELD_TYPE_FILTERS.map((filterValue) => {
      ...
      return {
        ...
        isActive,
        buttonClass:
          "oi-node-detail-panel-field-filter-button" +
          (isActive ? " is-active" : "")
      };
    });
  }
        ...
        badgeLabel:
          field.isCustom === true
            ...
            : field.isCustom === false
              ...
              : "—",
        badgeClass:
          "oi-node-detail-panel-field-badge" +
          (field.isCustom === true
            ? " is-custom"
            : field.isCustom === false
              ? " is-standard"
  ...
  async loadImpact(direction) {
    ...
    try {
      ...
      if (requestId === this.impactRequestId) {
        this.impactResult = null;
        this.impactErrorMessage =
          (error && error.body && error.body.message) ||
          "Something went wrong computing impact analysis.";
      }
    } finally {
      if (requestId === this.impactRequestId) {
        this.isLoadingImpact = false;
  ...
  /** The impact banner's Retry — recomputes the same direction the failed run asked for. */
  handleImpactRetry() {
    return this.loadImpact(this.impactDirection || "forward");
  }
  ...
  get impactDirectionLabel() {
    return this.impactDirection === "reverse"
  ...
  get impactTypeBreakdown() {
    ...
    for (const row of this.impactAffectedRows) {
      countByTypeLabel.set(
        row.typeLabel,
        (countByTypeLabel.get(row.typeLabel) || 0) + 1
      );
    }
  ...
  handleHighlightOnGraph() {
    ...
    this.dispatchEvent(
      new CustomEvent("highlightimpact", {
        detail: {
          nodeKey: this.impactResult.rootNodeKey,
          fragment: this.impactResult.subgraph
        }
      })
    );
  }
