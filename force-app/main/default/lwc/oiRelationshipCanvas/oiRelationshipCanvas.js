...
import {
  buildObjectRelationshipView,
  DEFAULT_VISIBLE_PER_LANE
} from "c/objectRelationshipView";
...
/** VisualDesignSpecification.md §6: "Connectors are orthogonal with rounded corners." Comfortably
 * smaller than the shortest adjacent segment a connector normally has (LABEL_SEGMENT_MIN=75,
 * FINAL_SEGMENT=75) so it only visibly shrinks (via buildElbowPath's own clamping) in unusually
 * tight row spacing, never overshoots into a self-intersecting curve. */
const CONNECTOR_CORNER_RADIUS = 14;
...
/**
 * Builds one connector's full "card row -> shared trunk -> center row" route as a single
 * orthogonal path with rounded corners at each turn, instead of three separate hard-right-angle
 * path elements (branch + shared trunk + final approach) meeting at unrounded T-junctions -- the
 * "not smooth and even" defect flagged directly against a real rendered screenshot, and the
 * connector-routing gap named in CurrentUIVisualGapAssessment.md §3 ("Orthogonal rounded paths
 * that visibly connect each card to the center"). Multiple rows sharing the same trunkX still
 * read as one merged trunk line, since their overlapping vertical runs are pixel-identical (same
 * X, same stroke) -- "shared trunks where useful" (VisualDesignSpecification.md §6) is preserved
 * without a separately-coordinated trunk path element to keep in sync.
 * Degenerates to a single straight H segment when startY === endY (a lone row already aligned
 * with the center has no vertical run, so nothing to round). The corner radius is clamped to the
 * shorter of its two adjacent segments so it can never overshoot into a self-intersecting curve
 * at unusually tight row spacing or label-segment width -- keeping routing deterministic for
 * identical inputs, per the same section's own requirement.
 */
function buildElbowPath(startX, startY, trunkX, endX, endY, radius) {
  if (startY === endY) {
    return `M ${startX} ${startY} H ${endX}`;
  }
  const hIn = Math.abs(trunkX - startX);
  const hOut = Math.abs(endX - trunkX);
  const vRun = Math.abs(endY - startY);
  const r = Math.min(radius, hIn, hOut, vRun / 2);
  const hInSign = Math.sign(trunkX - startX);
  const vSign = Math.sign(endY - startY);
  const hOutSign = Math.sign(endX - trunkX);
  const corner1X = trunkX - r * hInSign;
  const corner1Y = startY + r * vSign;
  const corner2Y = endY - r * vSign;
  const corner2X = trunkX + r * hOutSign;
  return `M ${startX} ${startY} H ${corner1X} Q ${trunkX} ${startY} ${trunkX} ${corner1Y} V ${corner2Y} Q ${trunkX} ${endY} ${corner2X} ${endY} H ${endX}`;
}
...
function buildConnectorLabelText(connector) {
  const hasFieldDetail =
    connector.fields.length > 0 && !!connector.fields[0].fieldApiName;
  const relationshipTypeLabel =
    connector.relationshipTypeLabel ||
    (connector.primaryRelationshipType === "MasterDetail"
      ? "Master-Detail"
      : "Lookup");
  let connectorLabel;
  if (hasFieldDetail) {
    connectorLabel =
      connector.relationshipCount > 1
        ? `${connector.relationshipCount} ${relationshipTypeLabel} Relationships`
        : `${connector.fields[0].fieldApiName} · ${relationshipTypeLabel}`;
  } else {
    connectorLabel =
      connector.relationshipCount > 1
        ? `${connector.relationshipCount} ${relationshipTypeLabel}`
...
export default class OiRelationshipCanvas extends LightningElement {
  ...
  get view() {
    ...
    this._viewCache = {
      nodes: this.nodes,
      edges: this.edges,
      centerNodeKey: this.centerNodeKey,
      mode: this.mode,
      value
    };
    return value;
  ...
  get rootObjectTypeLabel() {
    ...
    return this.isRecordMode
      ? `${this.rootObject.secondaryKey} Record`
      : this.rootObject.isCustom
        ? "Custom Object"
        : "Standard Object";
  }
  ...
  get incomingLaneTitle() {
    return this.isRecordMode
      ? "Records Referencing This Record"
      : "Objects Referencing This Object";
  }

  get outgoingLaneTitle() {
    return this.isRecordMode
      ? "Records This Record References"
      : "Objects This Object References";
  }

  get incomingLaneInfoText() {
    return this.isRecordMode
      ? "Records with a lookup pointing to this record, and child records related to it"
      : "Objects with a Lookup or Master-Detail field pointing to this object";
  }

  get outgoingLaneInfoText() {
    return this.isRecordMode
      ? "Records this record looks up to, or is a child of"
      : "Fields on this object that look up or master-detail to another object";
  }
  ...
  get incomingEmptyMessage() {
    return this.isRecordMode
      ...
      : "No objects reference this object.";
  }

  get outgoingEmptyMessage() {
    return this.isRecordMode
      ? "This record has no parent lookups or child records."
      : "This object references no other objects.";
  }
  ...
  get viewOptionsLabel() {
    ...
    const match = VIEW_OPTION_CHOICES.find(
      (choice) => choice.value === this.viewOptionMode
    );
  ...
  get viewOptionChoices() {
    return VIEW_OPTION_CHOICES.map((choice) => ({
      ...choice,
      buttonClass:
        "oi-orc-view-options-item" +
        (this.viewOptionMode === choice.value ? " is-active" : "")
    }));
  }
  ...
  applyFitToScreen() {
    ...
    const scale = Math.min(
      rect.width / d.canvasWidth,
      rect.height / d.canvasHeight
    );
  ...
  handleZoomIn() {
    ...
    this.zoomLevel = clamp(
      Math.round((this.zoomLevel + ZOOM_STEP) * 100) / 100,
      MIN_ZOOM,
      MAX_ZOOM
    );
  }
  ...
  handleExportDiagram() {
    ...
    const fileSafeLabel = (
      this.rootObject ? this.rootObject.label : "object"
    ).replace(/[^a-z0-9-_]+/gi, "-");
  ...
  get isOutgoingEmptyByBusinessFilter() {
    return (
      !this.hasOutgoing &&
      this.isBusinessMode &&
      this.view.outgoingRelationships.length > 0
    );
  }
   ...
   * force/random layout. An "org-chart trunk" per lane: each row's own path (buildElbowPath)
   * branches off its card into a shared vertical trunk X, then makes one final approach into
   * the center card, with a rounded corner at each turn instead of a hard right angle — this
   * keeps routing orthogonal, non-crossing, and visually smooth regardless of how many rows
   * converge, while every branch still carries its own label.
   */
  get diagram() {
    ...
    const canvasWidth =
      MARGIN_X * 2 +
      CARD_WIDTH * 2 +
      incomingLabelSegment +
      outgoingLabelSegment +
      FINAL_SEGMENT * 2 +
      CENTER_WIDTH;
    const centerX =
      MARGIN_X + CARD_WIDTH + incomingLabelSegment + FINAL_SEGMENT;
    ...
    const selfHeight =
      self.length > 0 ? self.length * SELF_ENTRY_HEIGHT + 20 : 0;
    const canvasHeight = diagramHeight + selfHeight;

    const incomingCards = incoming.map((connector, index) => {
      const rowY = this.laneRowCenterY(
        index,
        incomingRowSlots,
        bodyHeight,
        TOP_MARGIN
      );
      return this.buildIncomingEntry(
        connector,
        index,
        rowY,
        incomingCardX,
    ...
    const outgoingCards = outgoing.map((connector, index) => {
      ...
      return this.buildOutgoingEntry(
        connector,
        index,
        rowY,
        outgoingCardX,
    ...
    const outgoingShowMore = this.hasMoreOutgoing
      ? this.buildShowMoreEntry(
          ...
          outgoingCardX,
          this.showMoreOutgoingLabel
        )
      : null;

    const selfEntries = self.map((connector, index) =>
      this.buildSelfEntry(
        connector,
        index,
        centerX,
    ...
    const activeCard =
      [...incomingCards, ...outgoingCards].find(
        (card) => card.connectorKey === this.selectedConnectorKey
      ) || null;
  ...
  buildIncomingEntry(
    ...
    const branchPath = buildElbowPath(
      cardX + CARD_WIDTH,
      rowY,
      trunkX,
      centerLeftX,
  ...
  decorateConnector(connector, laneKey, index) {
    const { connectorLabel, relationshipTypeLabel } =
      buildConnectorLabelText(connector);
    ...
    return {
      ...
      connectorLabel,
      relationshipTypeLabel,
      counterpartTypeLabel: this.counterpartTypeLabel(
        connector.counterpartObject
      ),
      selfCaptionSubtitle: this.selfCaptionSubtitle(),
      lineClass: this.connectorLineClass(
        connector,
        laneKey,
        isActive,
        isDimmed
      ),
      labelClass: this.connectorLabelClass(
        connector,
        laneKey,
        isActive,
        isDimmed
  ...
  counterpartTypeLabel(counterpartObject) {
    return this.isRecordMode
      ? `${counterpartObject.secondaryKey} Record`
      : counterpartObject.isCustom
        ? "Custom Object"
        : "Standard Object";
  }
  ...
  findConnector(connectorKey) {
    const all = [
      ...this.view.incomingRelationships,
      ...this.view.outgoingRelationships,
      ...this.view.selfRelationships
    ];
    return (
      all.find((connector) => connector.connectorKey === connectorKey) || null
    );
  }
  ...
  handleCardOpen(event) {
    ...
    this.dispatchEvent(
      new CustomEvent("open", {
        detail: { nodeKey: event.currentTarget.dataset.nodeKey }
      })
    );
  }
  ...
  handleExploreFromHere(event) {
    ...
    const nodeKey = event.currentTarget.dataset.nodeKey;
    this.dispatchEvent(
      new CustomEvent("explorefromhere", { detail: { nodeKey } })
    );
  }
  ...
  handleConnectorClick(event) {
    ...
    this.dispatchEvent(
      new CustomEvent("edgeclick", {
        detail: { connector, rootObject: this.rootObject }
      })
    );
  }
  ...
  syncSvgDefReferences() {
    ...
    this.template
      .querySelectorAll('[data-id="orc-arrow-path"]')
      .forEach((path) => {
