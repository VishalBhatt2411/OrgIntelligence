...
function objectNode(nodeKey, label, secondaryKey) {
    return { nodeKey, typeKey: OBJECT_TYPE_KEY, label, secondaryKey, iconName: 'standard:account', colorToken: 'neutral', typeLabel: 'Object' };
}
...
function apexClassNode(nodeKey, label) {
    return { nodeKey, typeKey: 'SalesforceMetadata.ApexClass', label, secondaryKey: label, iconName: 'standard:apex', colorToken: 'neutral' };
}
...
function baseFixture() {
    return {
        nodes: [objectNode('account', 'Account', 'Account'), objectNode('user', 'User', 'User'), objectNode('opportunity', 'Opportunity', 'Opportunity'), fieldNode('accountManagerId', 'AccountManagerId'), fieldNode('accountId', 'AccountId')],
        edges: [
...
function renderCanvas({ nodes, edges, centerNodeKey, mode }) {
    const element = createElement('c-oi-relationship-canvas', { is: OiRelationshipCanvas });
    element.nodes = nodes;
    element.edges = edges;
    if (mode) {
        element.mode = mode;
...
function recordNode(objectApiName, recordId, label) {
    return { nodeKey: recordKey(objectApiName, recordId), typeKey: `SalesforceRecord.${objectApiName}`, label, secondaryKey: objectApiName, iconName: 'standard:record', colorToken: 'neutral' };
}

function recordEdge(typeKey, sourceKey, targetKey) {
    return { edgeKey: `${sourceKey}-${targetKey}`, typeKey, sourceNodeKey: sourceKey, targetNodeKey: targetKey };
}
...
describe('c-oi-relationship-canvas', () => {
    ...
    describe('Object-only card filter', () => {
        it('renders only Object cards, never Field/ApexClass/ApexTrigger/Flow/PermissionSet cards', () => {
            ...
            fixture.edges.push({ edgeKey: 'e-apex', typeKey: 'SalesforceMetadata.EXECUTES_ON', sourceNodeKey: 'apexClass1', targetNodeKey: 'account' });
            const element = renderCanvas({ ...fixture, centerNodeKey: 'account' });

            return Promise.resolve().then(() => {
                const allCardTitles = [...element.shadowRoot.querySelectorAll('.oi-orc-card-title')].map((el) => el.textContent);
                ...
                expect(element.shadowRoot.querySelectorAll('[data-id="incoming-card"], [data-id="outgoing-card"]').length).toBeGreaterThan(0);
            });
        });
    });
    ...
    describe('lane layout', () => {
        it('renders the centered object prominently in the center card', () => {
            ...
            return Promise.resolve().then(() => {
                const centerCard = element.shadowRoot.querySelector('[data-id="center-card"]');
        ...
        it('places an object that references the center among the incoming cards, not the outgoing cards', () => {
            ...
            return Promise.resolve().then(() => {
                const incomingCards = [...element.shadowRoot.querySelectorAll('[data-id="incoming-card"]')].map((el) => el.textContent).join(' ');
                const outgoingCards = [...element.shadowRoot.querySelectorAll('[data-id="outgoing-card"]')].map((el) => el.textContent).join(' ');
        ...
        it('renders a self-referencing relationship as its own dedicated self card, not among the incoming/outgoing cards', () => {
            ...
            return Promise.resolve().then(() => {
                const selfCard = element.shadowRoot.querySelector('[data-id="self-card"]');
                expect(selfCard).not.toBeNull();
                expect(selfCard.textContent).toContain('Account (Self)');
                const selfConnector = element.shadowRoot.querySelector('[data-id="self-connector"]');
                expect(selfConnector.textContent).toContain('ParentId');
                expect(element.shadowRoot.querySelector('[data-id="incoming-empty"]')).not.toBeNull();
                expect(element.shadowRoot.querySelector('[data-id="outgoing-empty"]')).not.toBeNull();
            });
        });
        ...
        it('shows an honest empty-lane message when there are no incoming/outgoing relationships', () => {
            ...
            const element = renderCanvas({ nodes, edges: [], centerNodeKey: 'lonely' });

            return Promise.resolve().then(() => {
                expect(element.shadowRoot.querySelector('[data-id="incoming-empty"]').textContent).toContain('No objects reference');
    ...
    describe('relationship connector correctness', () => {
        it('shows the field API name and relationship type on the connector', () => {
            ...
            return Promise.resolve().then(() => {
                const outgoingConnector = element.shadowRoot.querySelector('[data-id="outgoing-connector"]');
                expect(outgoingConnector.textContent).toContain('AccountManagerId');
                expect(outgoingConnector.textContent).toContain('Lookup');

                const incomingConnector = element.shadowRoot.querySelector('[data-id="incoming-connector"]');
        ...
        it('aggregates multiple relationship fields between the same object pair into one connector stating the count plainly, never a cryptic "+N"', () => {
            const nodes = [objectNode('account', 'Account', 'Account'), objectNode('user', 'User', 'User'), fieldNode('accountManagerId', 'AccountManagerId'), fieldNode('regionalDirectorId', 'RegionalDirectorId')];
            const edges = [
                hasFieldEdge('account', 'accountManagerId'),
                lookupEdge('accountManagerId', 'user', 'AccountManagerId'),
            ...
            return Promise.resolve().then(() => {
                const outgoingConnector = element.shadowRoot.querySelector('[data-id="outgoing-connector"]');
                expect(outgoingConnector.textContent).toContain('2 Lookup Relationships');
        ...
        it('clicking an aggregated connector emits edgeclick with every field, so the caller can open the full detail — no separate inline expand affordance to keep in sync', () => {
            ...
            return Promise.resolve().then(() => {
                element.shadowRoot.querySelector('[data-id="outgoing-connector"]').click();
                expect(handler).toHaveBeenCalledTimes(1);
                const fields = handler.mock.calls[0][0].detail.connector.fields.map((f) => f.fieldApiName).sort();
    ...
    describe('system vs. business relationship visibility', () => {
        function systemAndBusinessFixture() {
            return {
                nodes: [objectNode('account', 'Account', 'Account'), objectNode('user', 'User', 'User'), objectNode('territory', 'Territory', 'Territory'), fieldNode('ownerId', 'OwnerId'), fieldNode('territoryId', 'TerritoryId')],
                edges: [hasFieldEdge('account', 'ownerId'), lookupEdge('ownerId', 'user', 'OwnerId'), hasFieldEdge('account', 'territoryId'), lookupEdge('territoryId', 'territory', 'TerritoryId')]
            };
        }

        function outgoingCardText(element) {
            return [...element.shadowRoot.querySelectorAll('[data-id="outgoing-card"]')].map((el) => el.textContent).join(' ');
        }

        it('defaults to Business relationships only, hiding a pure system relationship (OwnerId)', () => {
            const element = renderCanvas({ ...systemAndBusinessFixture(), centerNodeKey: 'account' });
    ...
    describe('interactions', () => {
        ...
        it('shows a contextual "Explore From Here" action only after a card is selected, never as a permanent per-card link', () => {
            ...
            return Promise.resolve()
                .then(() => {
                    expect(element.shadowRoot.querySelector('[data-id="explore-from-here"]')).toBeNull();
                    element.shadowRoot.querySelector('[data-id="outgoing-card"]').click();
                    return Promise.resolve();
                })
                .then(() => {
                    const exploreButton = element.shadowRoot.querySelector('[data-id="explore-from-here"]');
        ...
        it('reveals additional connectors client-side via "show more" without emitting any fetch-triggering event', () => {
            ...
            for (const letter of objectLetters) {
                nodes.push(objectNode('obj' + letter, 'Object ' + letter, 'Object_' + letter + '__c'));
                nodes.push(fieldNode('field' + letter, 'Field' + letter + 'Id'));
                edges.push(hasFieldEdge('account', 'field' + letter));
                edges.push(lookupEdge('field' + letter, 'obj' + letter, 'Field' + letter + 'Id'));
            }
            const element = renderCanvas({ nodes, edges, centerNodeKey: 'account' });
            ...
            return Promise.resolve()
                .then(() => {
                    expect(element.shadowRoot.querySelectorAll('[data-id="outgoing-card"]')).toHaveLength(6);
                    const showMore = element.shadowRoot.querySelector('[data-id="show-more-outgoing"]');
                ...
                .then(() => {
                    expect(element.shadowRoot.querySelectorAll('[data-id="outgoing-card"]')).toHaveLength(8);
                    expect(element.shadowRoot.querySelector('[data-id="show-more-outgoing"]')).toBeNull();
    ...
    describe('accessibility', () => {
        ...
        it('every neighbor card is keyboard-focusable and activatable with Enter', () => {
            ...
            return Promise.resolve().then(() => {
                const card = element.shadowRoot.querySelector('[data-id="outgoing-card"]');
        ...
        it('shrinks the SVG\'s own rendered width/height with zoom-out instead of a CSS transform, so the diagram-wrapper\'s scrollable area shrinks in lockstep and no dead space opens up around the self-relationship lane', () => {
            ...
            return Promise.resolve()
                ...
                .then(({ fullWidth, fullHeight }) => {
                    ...
                    expect(svg.getAttribute('viewBox')).toBe(`0 0 ${fullWidth} ${fullHeight}`);
    ...
    describe('Record analyze mode (ADR-0024 — shares this canvas with Object mode)', () => {
        function recordFixture() {
            return {
                ...
                edges: [recordEdge(RECORD_CHILD_OF, recordKey('Account', '001x1'), recordKey('Contact', '003x1')), recordEdge(RECORD_LOOKUP_TO, recordKey('Account', '001x1'), recordKey('User', '005x1'))]
            };
        }

        it('renders the centered record in the center card, labeled as a Record rather than a Standard/Custom Object', () => {
            const element = renderCanvas({ ...recordFixture(), centerNodeKey: recordKey('Account', '001x1'), mode: 'Record' });
        ...
        it('places a child record among the incoming cards and a parent lookup among the outgoing cards, under Record-mode lane titles', () => {
            ...
            return Promise.resolve().then(() => {
                expect(element.shadowRoot.querySelector('[data-id="incoming-card"]').textContent).toContain('Jane Doe');
                expect(element.shadowRoot.querySelector('[data-id="outgoing-card"]').textContent).toContain('Jane Admin');
                const laneTitles = [...element.shadowRoot.querySelectorAll('.oi-orc-lane-title')].map((el) => el.textContent).join(' ');
        ...
        it('hides the Business/System/All toggle entirely — Record mode has no field-level detail to classify System vs. Business by', () => {
            ...
            return Promise.resolve().then(() => {
                expect(element.shadowRoot.querySelector('.oi-orc-visibility-toggle-group')).toBeNull();
                expect(element.shadowRoot.querySelector('[data-id="legend"]').textContent).not.toContain('System Relationship');
            });
        });
        ...
        it('treats a same-object-type counterpart record as a dedicated self card naming the ACTUAL related record, not the center\'s own name', () => {
            ...
            const edges = [recordEdge(RECORD_LOOKUP_TO, recordKey('Account', '001x1'), recordKey('Account', '001x2'))];
            const element = renderCanvas({ nodes, edges, centerNodeKey: recordKey('Account', '001x1'), mode: 'Record' });

            return Promise.resolve().then(() => {
                const selfCard = element.shadowRoot.querySelector('[data-id="self-card"]');
                expect(selfCard.textContent).toContain('Acme Corp — West (Self)');
                expect(selfCard.textContent).not.toContain('Acme Corp (Self)');
                expect(element.shadowRoot.querySelector('[data-id="incoming-card"]')).toBeNull();
                expect(element.shadowRoot.querySelector('[data-id="outgoing-card"]')).toBeNull();
            });
        });
        ...
        it('"Explore From Here" on a related record card emits the RECORD nodeKey, for oiGraphExplorer to re-center via selectAndCenterRecord', () => {
            ...
            return Promise.resolve()
                ...
                .then(() => {
                    ...
                    exploreButton.click();
                    expect(exploreHandler).toHaveBeenCalledTimes(1);
                    expect(exploreHandler.mock.calls[0][0].detail.nodeKey).toBe(recordKey('User', '005x1'));
                });
        });
    });
    ...
    describe('re-centering resets local view state (GraphUI.md §11\'s "a fresh center is a fresh view", live-org-validated regression)', () => {
        it('resets the Business/System/All toggle and any expanded connector back to their defaults when centerNodeKey changes', () => {
            ...
            return Promise.resolve()
                ...
                .then(() => {
                    expect(element.shadowRoot.querySelector('[data-mode="all"]').getAttribute('aria-pressed')).toBe('true');
                    element.centerNodeKey = 'user';
                    element.nodes = [objectNode('user', 'User', 'User'), objectNode('account', 'Account', 'Account')];
                    element.edges = [];
                    return Promise.resolve();
                })
                .then(() => {
                    expect(element.shadowRoot.querySelector('[data-mode="business"]').getAttribute('aria-pressed')).toBe('true');
                    expect(element.shadowRoot.querySelector('[data-mode="all"]').getAttribute('aria-pressed')).toBe('false');
                });
        });
    });
});
