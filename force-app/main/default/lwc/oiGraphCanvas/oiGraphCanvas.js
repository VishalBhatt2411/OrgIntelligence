...
export default class OiGraphCanvas extends LightningElement {
  ...
  get hostClass() {
    return (
      "oi-graph-canvas-container" +
      (this.isMaximized ? " oi-graph-canvas-is-maximized" : "")
    );
  }
  ...
  get viewBox() {
    const { x, y, width, height } = this.viewBoxRect(
      this.panX,
      this.panY,
      this.zoom
    );
  ...
  get disableCenterOnSelected() {
    return (
      ...
      !this.positionedNodes.some((n) => n.nodeKey === this.selectedNodeKey)
    );
  }
  ...
  get renderableNodes() {
    ...
    const visible = this.positionedNodes.filter((node) =>
      boxIntersectsWindow(node, window)
    );
  ...
  buildParallelEdgeIndex(edges, resolveDisplayKey) {
    ...
    for (const memberKeys of groups.values()) {
      memberKeys.forEach((edgeKey, index) =>
        indexByEdgeKey.set(edgeKey, { index, count: memberKeys.length })
      );
    }
  ...
  get renderableEdges() {
    ...
    const renderableKeys = new Set(
      this.renderableNodes.map((node) => node.nodeKey)
    );
    ...
    const pathKeys = this.activePathKeySet;
    const parallelIndex = this.buildParallelEdgeIndex(
      this.edges,
      resolveDisplayKey
    );
    const rendered = [];
    for (const edge of this.edges || []) {
      ...
      const parallel = parallelIndex.get(edge.edgeKey) || {
        index: 0,
        count: 1
      };
      const parallelOffset =
        (parallel.index - (parallel.count - 1) / 2) *
        PARALLEL_EDGE_SEPARATION_PX;
      ...
      const lineStyle = KNOWN_EDGE_LINE_STYLES.has(edge.lineStyle)
        ? edge.lineStyle
        : "solid";
      ...
      const isOnActivePath =
        !!pathKeys && pathKeys.has(sourceKey) && pathKeys.has(targetKey);
      ...
      const labelPosition = shortLabel
        ? edgeLabelPosition(
            source.cx,
            source.cy,
            target.cx,
            target.cy,
            parallelOffset,
            footprintRadius(source),
            footprintRadius(target)
          )
        : null;
      rendered.push({
    ...
    for (const clusterEdge of layout.clusterEdges) {
      ...
      if (
        !renderableKeys.has(clusterEdge.sourceNodeKey) &&
        !renderableKeys.has(clusterEdge.targetNodeKey)
      ) {
        continue;
      }
  ...
  buildEdgeShortLabel(edge) {
    ...
    return edge.viaFieldApiName
      ? `${edge.viaFieldApiName} · ${relationship}`
      : relationship;
  }
  ...
  get miniMap() {
    ...
    const dots = positioned.map((n) => {
      ...
      return {
        ...
        cx: p.x,
        cy: p.y,
        isCenter: n.nodeKey === this.centerNodeKey
      };
    });
  ...
  getLayout() {
    ...
    if (
      cache &&
      cache.base === base &&
      cache.manualOffsets === this.manualOffsets
    ) {
      return cache.result;
    }
    const result =
      this.manualOffsets.size === 0
        ? base
        : applyManualOffsets(base, this.manualOffsets);
  ...
  computeBaseLayout() {
    ...
    const resolveDisplayKey = (key) => fieldOwnerByFieldKey.get(key) || key;
    const fieldsByOwnerKey = this.buildFieldListsByOwner(
      fieldOwnerByFieldKey,
      nodeByKey
    );
    const displayNodes = nodes.filter(
      (node) => !fieldOwnerByFieldKey.has(node.nodeKey)
    );
    ...
    const effectiveCenterKey = this.centerNodeKey
      ? resolveDisplayKey(this.centerNodeKey)
      : this.centerNodeKey;
    ...
    const { displayItems, clusterEdges } = this.groupIntoClusters(
      displayNodes,
      byKey,
      ringByKey,
      parentByKey,
     ...
     */
    const edgeRoleByPair = this.buildEdgeRoleIndex(
      nodeByKey,
      resolveDisplayKey
    );
    for (const item of displayItems) {
      ...
      const relationship = this.resolveRelationshipContext(
        roleSourceKey,
        parentKey,
        edgeRoleByPair
      );
    ...
    for (const clusterEdge of clusterEdges) {
      ...
      layoutAdjacency
        .get(clusterEdge.sourceNodeKey)
        .push(clusterEdge.targetNodeKey);
      layoutAdjacency
        .get(clusterEdge.targetNodeKey)
        .push(clusterEdge.sourceNodeKey);
    }
    ...
    for (const ring of sortedRings) {
      ...
      const footprints = ringItems.map((item) =>
        footprintByKey.get(item.nodeKey)
      );
      ...
      const totalSpan = footprints.reduce(
        (sum, f) => sum + f.width + NODE_GAP,
        0
      );
      const requiredRadius = totalSpan / (2 * Math.PI);
      const minRadiusForSpacing =
        previousOuterRadius + RING_GAP + maxHalfHeight;
    ...
    for (let sweep = 0; sweep < RELAXATION_SWEEPS; sweep++) {
      ...
      for (const ring of sortedRings) {
        ...
        const reordered = sortRingByBarycenter(
          ringItems,
          ring,
          angleByKey,
          ringOfKey,
  ...
  groupIntoClusters(nodes, byKey, ringByKey, parentByKey, effectiveCenterKey) {
    ...
    const singles = [];
    for (const node of nodes) {
      const ring = ringByKey.has(node.nodeKey)
        ? ringByKey.get(node.nodeKey)
        : node.nodeKey === effectiveCenterKey
          ? 0
          : 1;
      if (ring === 0) {
  ...
  buildEdgeRoleIndex(nodeByKey, resolveDisplayKey) {
    ...
    for (const edge of this.edges || []) {
      ...
      if (resolvedSource !== edge.sourceNodeKey) {
        ...
        const apiNameSegments =
          fieldNode && fieldNode.secondaryKey
            ? fieldNode.secondaryKey.split(".")
            : null;
        viaFieldApiName = apiNameSegments
          ? apiNameSegments[apiNameSegments.length - 1]
          : null;
      }
  ...
  resolveRelationshipContext(nodeKey, parentKey, edgeRoleByPair) {
    ...
    const role =
      (anchorIsSource ? entry.targetRoleLabel : entry.sourceRoleLabel) ||
      "Related To";
  ...
  handleNodeOpen(event) {
    this.dispatchEvent(
      new CustomEvent("open", { detail: { nodeKey: event.detail.nodeKey } })
    );
  }
  ...
  syncSvgDefReferences() {
    const pattern = this.template.querySelector(
      '[data-id="canvas-grid-pattern"]'
    );
    const gridRect = this.template.querySelector(
      '[data-id="canvas-grid-rect"]'
    );
    if (pattern && gridRect) {
    ...
    const marker = this.template.querySelector(
      '[data-id="canvas-arrow-marker"]'
    );
    if (marker) {
      /**
      ...
      this.template
        .querySelectorAll('[data-id="graph-edge-path"]')
        .forEach((path) => {
  ...
  handleWheel(event) {
    ...
    const svgX =
      before.x + ((event.clientX - rect.left) / rect.width) * before.width;
    const svgY =
      before.y + ((event.clientY - rect.top) / rect.height) * before.height;

    const delta = event.deltaY > 0 ? -0.12 : 0.12;
    const newZoom = Math.min(
      MAX_ZOOM,
      Math.max(MIN_ZOOM, this.zoom * (1 + delta))
    );
    this.applyZoomAnchoredAt(
      svgX,
      svgY,
      newZoom,
  ...
  animateZoomAnchored(svgX, svgY, fracX, fracY, targetZoom) {
    ...
    const step = () => {
      ...
      if (t < 1) {
        // eslint-disable-next-line @lwc/lwc/no-async-operation -- a frame-synced easing loop is intrinsic to a smooth zoom animation; no timer-free alternative exists for this.
  ...
  handleFitToScreenClick() {
    ...
    const targetZoom = Math.min(
      ...
      Math.max(
        MIN_ZOOM,
        Math.min(
          VIEW_WIDTH / (contentWidth * FIT_TO_SCREEN_PADDING),
          VIEW_HEIGHT / (contentHeight * FIT_TO_SCREEN_PADDING)
        )
      )
    );
  ...
  handleCenterOnSelectedClick() {
    this.cancelMomentum();
    const selected = this.positionedNodes.find(
      (n) => n.nodeKey === this.selectedNodeKey
    );
    if (!selected) {
      return;
    }
    this.animateTo(
      selected.cx - VIEW_WIDTH / 2,
      selected.cy - VIEW_HEIGHT / 2,
      this.zoom
    );
  }
  ...
  animateTo(targetPanX, targetPanY, targetZoom) {
    ...
    const step = () => {
      ...
      if (t < 1) {
        // eslint-disable-next-line @lwc/lwc/no-async-operation -- a frame-synced easing loop is intrinsic to a smooth pan/zoom animation; no timer-free alternative exists for this.
        this.zoomAnimationHandle = requestAnimationFrame(step);
      } else {
        this.zoomAnimationHandle = null;
        this.dispatchEvent(
  ...
  recordPointerHistory(clientX, clientY) {
    ...
    while (
      ...
      now - this.pointerHistory[0].t > POINTER_HISTORY_WINDOW_MS
    ) {
  ...
  scheduleFrameFlush() {
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-operation -- coalescing rapid pointermove events to one flush per repaint is intrinsic to smooth drag/pan; no timer-free alternative exists for this.
  ...
  flushPendingMovement() {
    ...
    if (this.pendingNodeDelta && this.draggingNodeKey) {
      ...
      updated.set(this.draggingNodeKey, {
        dx: existing.dx + this.pendingNodeDelta.dx,
        dy: existing.dy + this.pendingNodeDelta.dy
      });
  ...
  startMomentum(vx, vy) {
    ...
    const step = () => {
      ...
      // eslint-disable-next-line @lwc/lwc/no-async-operation -- a frame-synced deceleration loop is intrinsic to momentum/flick coasting; no timer-free alternative exists for this.
...
function boxIntersectsWindow(item, window) {
  return (
    item.x + item.width >= window.minX &&
    item.x <= window.maxX &&
    item.y + item.height >= window.minY &&
    item.y <= window.maxY
  );
}
...
function assignSequentialRingAngles(ringItems, footprintByKey, angleByKey) {
  ...
  ringItems.forEach((item, index) => {
    ...
    const angleSlice =
      ((footprint.width + NODE_GAP) / totalSpan) * (2 * Math.PI);
...
function sortRingByBarycenter(
  ...
  const scored = ringItems.map((item, originalIndex) => {
    const neighborAngles = (layoutAdjacency.get(item.nodeKey) || [])
      .filter(
        (neighborKey) =>
          (ringOfKey.has(neighborKey) ? ringOfKey.get(neighborKey) : Infinity) <
          ring
      )
      .map((neighborKey) => angleByKey.get(neighborKey))
      .filter((angle) => angle !== undefined);
    const mean = circularMean(neighborAngles);
    const sortAngle = normalizeAngle(
      mean !== null ? mean : angleByKey.get(item.nodeKey)
    );
    return { item, originalIndex, sortAngle };
  });
  scored.sort(
    (a, b) => a.sortAngle - b.sortAngle || a.originalIndex - b.originalIndex
  );
...
function itemFootprint(item) {
   ...
   */
  const headerWidth = estimateLabelWidth(
    item.label,
    SCHEMA_CARD_LABEL_CHROME_PX,
    MIN_SCHEMA_CARD_WIDTH,
    MAX_SCHEMA_CARD_WIDTH
  );
  const subheaderWidth = item.secondaryKey
    ? estimateLabelWidth(
        item.secondaryKey,
        SCHEMA_CARD_SECONDARY_CHROME_PX,
        MIN_SCHEMA_CARD_WIDTH,
        MAX_SCHEMA_CARD_WIDTH
      )
    : MIN_SCHEMA_CARD_WIDTH;
  return {
    width: Math.max(headerWidth, subheaderWidth),
 ...
 * top of the stroke.
 *
 * The along-the-line position is a plain 50/50 midpoint only when both endpoints are similarly
 * sized. A raw midpoint crowds the bigger endpoint whenever the two differ a lot — a schema card
 * (SCHEMA_CARD_HEIGHT/MIN_SCHEMA_CARD_WIDTH, far bigger than a plain NODE_HEIGHT pill) connected
 * to a nearby small node left its own label sitting right on the schema card's border (live-org
 * screenshot: the Calendar object's "ParentRecord Lookup To" labels overlapping the Calendar
 * card itself). `sourceRadius`/`targetRadius` (each endpoint's own half-footprint) bias the split
 * point away from the larger box; clamped to [0.35, 0.65] so no single edge collapses onto either
 * endpoint outright.
 */
function edgeLabelPosition(
  ...
  const dy = y2 - y1;
  const length = Math.hypot(dx, dy) || 1;
  const totalRadius = sourceRadius + targetRadius;
  const t =
    totalRadius > 0 ? clamp(sourceRadius / totalRadius, 0.35, 0.65) : 0.5;
  const midX = x1 + dx * t;
  const midY = y1 + dy * t;
  const bow =
    Math.min(length * 0.12, 24) + EDGE_LABEL_OFFSET_PX + parallelOffset;
  return {
...
/** A box's own rough clearance radius — half its larger dimension — used only to bias where an
 * edge's label sits (see edgeLabelPosition), never for layout/placement itself. */
function footprintRadius(node) {
  return Math.max(node.width || 0, node.height || 0) / 2;
}
